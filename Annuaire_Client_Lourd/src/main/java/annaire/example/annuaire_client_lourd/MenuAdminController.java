package annaire.example.annuaire_client_lourd;

import javafx.event.ActionEvent;
import javafx.fxml.FXML;
import javafx.fxml.FXMLLoader;
import javafx.geometry.Rectangle2D;
import javafx.scene.Parent;
import javafx.scene.Scene;
import javafx.scene.control.Button;
import javafx.scene.image.ImageView;
import javafx.scene.input.MouseEvent;
import javafx.stage.Screen;
import javafx.stage.Stage;

import java.io.IOException;

public class MenuAdminController {

    private void centerSceneOnPrimaryScreen(Stage stage) {
        // Obtenir les dimensions de l'écran principal
        Screen screen = Screen.getPrimary();
        Rectangle2D bounds = screen.getVisualBounds();

        // Centrer la scène sur l'écran
        double centerX = bounds.getMinX() + (bounds.getWidth() - stage.getWidth()) / 2;
        double centerY = bounds.getMinY() + (bounds.getHeight() - stage.getHeight()) / 2;

        // Définir la position de la scène
        stage.setX(centerX);
        stage.setY(centerY);
    }
    @FXML
    public Button buttonSalariesAdmin;
    @FXML
    public void goToSalariesAdmin(ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("salariesAdmin.fxml"));
            Parent root = loader.load();


            // Obtenez la hauteur de l'écran
            double screenHeight = Screen.getPrimary().getVisualBounds().getHeight();
            double screenWidth = Screen.getPrimary().getVisualBounds().getWidth();

            // Définissez la hauteur de la scène à 96% de l'écran
            double sceneHeight = screenHeight * 0.96;
            double sceneWidth = screenWidth * 1;
            Scene scene = new Scene(root, sceneWidth, sceneHeight);


            Stage stage = (Stage) buttonSalariesAdmin.getScene().getWindow();
            stage.setScene(scene);
            stage.show();

            // Centrer la nouvelle scène
            centerSceneOnPrimaryScreen(stage);


        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    @FXML
    public Button buttonQuitAdmin;

    @FXML
    public void QuitAdmin(ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("salaries.fxml"));
            Parent root = loader.load();


            // Obtenez la hauteur de l'écran
            double screenHeight = Screen.getPrimary().getVisualBounds().getHeight();
            double screenWidth = Screen.getPrimary().getVisualBounds().getWidth();

            // Définissez la hauteur de la scène à 96% de l'écran
            double sceneHeight = screenHeight * 0.96;
            double sceneWidth = screenWidth * 1;
            Scene scene = new Scene(root, sceneWidth, sceneHeight);


            Stage stage = (Stage) buttonQuitAdmin.getScene().getWindow();
            stage.setScene(scene);
            stage.show();

            // Centrer la nouvelle scène
            centerSceneOnPrimaryScreen(stage);


        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    @FXML
    public Button buttonSitesAdmin;
    @FXML
    public void goToSitesAdmin(ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("sitesAdmin.fxml"));
            Parent root = loader.load();


            // Obtenez la hauteur de l'écran
            double screenHeight = Screen.getPrimary().getVisualBounds().getHeight();
            double screenWidth = Screen.getPrimary().getVisualBounds().getWidth();

            // Définissez la hauteur de la scène à 96% de l'écran
            double sceneHeight = screenHeight * 0.96;
            double sceneWidth = screenWidth * 1;
            Scene scene = new Scene(root, sceneWidth, sceneHeight);


            Stage stage = (Stage) buttonSitesAdmin.getScene().getWindow();
            stage.setScene(scene);
            stage.show();

            // Centrer la nouvelle scène
            centerSceneOnPrimaryScreen(stage);


        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    @FXML
    public Button buttonServicesAdmin;
    @FXML
    public void goToServicesAdmin(ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("servicesAdmin.fxml"));
            Parent root = loader.load();


            // Obtenez la hauteur de l'écran
            double screenHeight = Screen.getPrimary().getVisualBounds().getHeight();
            double screenWidth = Screen.getPrimary().getVisualBounds().getWidth();

            // Définissez la hauteur de la scène à 96% de l'écran
            double sceneHeight = screenHeight * 0.96;
            double sceneWidth = screenWidth * 1;
            Scene scene = new Scene(root, sceneWidth, sceneHeight);


            Stage stage = (Stage) buttonServicesAdmin.getScene().getWindow();
            stage.setScene(scene);
            stage.show();

            // Centrer la nouvelle scène
            centerSceneOnPrimaryScreen(stage);


        } catch (IOException e) {
            e.printStackTrace();
        }
    }

}
